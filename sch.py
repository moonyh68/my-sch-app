...
def insert_task_range(start_date, end_date, start_time, end_time, title, memo, is_done=0, is_meeting=0, meeting_notes=""):
...
def update_task_full(target_id, task_date, start_time, end_time, title, memo, is_meeting, meeting_notes):
    ...
    if mask.any():
        ...
        cal_success, new_event_id, cal_err = update_google_calendar_event(
            old_event_id, task_date, start_time, end_time, title, memo
        )
...
def update_task_range(target_id, start_date, end_date, start_time, end_time, title, memo, is_meeting=0, meeting_notes=""):
    succ, err = delete_task(target_id)
    if not succ:
        return False, err
    return insert_task_range(start_date, end_date, start_time, end_time, title, memo, 0, is_meeting, meeting_notes)
...
def open_day_modal(target_date):
    ...
    if not all_df.empty:
        ...
        if not tasks_df.empty:
            tasks_df["sort_time"] = tasks_df["start_time"].apply(format_sort_time)
    ...
    if not tasks_df.empty:
        ...
        for _, row in tasks_df.iterrows():
            ...
            with st.expander(f"[{status_text}] {row['start_time']}~{row['end_time']} | {row['title']}", expanded=False):
                ...
                edit_mode = st.radio(
                    "날짜 수정 방식", ["단일 날짜 변경", "연속 기간으로 확장/변경"], horizontal=True, key=f"e_mode_{row_id}"
                )
                ...
                if edit_mode == "단일 날짜 변경":
                    ...
                    edit_range = st.date_input(
                        "기간 선택 (시작일 ~ 종료일)",
                        value=(row_date_val, row_date_val + datetime.timedelta(days=2)),
                        key=f"e_range_{row_id}",
                    )
                ...
                edit_meeting_notes = ""
                if edit_is_meeting:
                    edit_meeting_notes = st.text_area(
                        "회의 내용 및 결정 사항",
                        value=row["meeting_notes"] if pd.notna(row["meeting_notes"]) else "",
                        key=f"e_notes_{row_id}",
                    )
...
for idx, (day_name, color_code) in enumerate(days_of_week):
    cols[idx].markdown(
        f"<div style='text-align: center; color: {color_code}; font-weight: 800; font-size: 14px; padding: 2px 0; border: none; margin-bottom: 2px;'>{day_name}</div>",
        unsafe_allow_html=True,
    )
