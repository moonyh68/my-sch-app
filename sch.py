...
# ---------------------------------------------------------
# 1. 구글 시트 데이터베이스 연동 함수 (날짜 정규화 적용)
# ---------------------------------------------------------
...
def fetch_all_tasks():
    try:
        ...
        # [디버깅 보완] task_date를 YYYY-MM-DD 정규 포맷으로 통일
...
def fetch_month_tasks(year, month):
    ...
    # [디버깅 보완] 연도와 월 조건 매칭을 datetime으로 안전하게 파싱하여 필터링
    try:
        df['dt_temp'] = pd.to_datetime(df['task_date'], errors='coerce')
        filtered_df = df[(df['dt_temp'].dt.year == year) & (df['dt_temp'].dt.month == month)].copy()
        return filtered_df.drop(columns=['dt_temp'])
    except Exception:
        ...
        for _, row in tasks_df.iterrows():
            ...
            with st.expander(f"[{status_text}] {row['start_time']}~{row['end_time']} | {row['title']}", expanded=False):
                ...
                if edit_mode == "단일 날짜 변경":
                    ...
                    edit_date = None
                    edit_range = st.date_input("기간 선택 (시작일 ~ 종료일)", value=(row_date_val, row_date_val + datetime.timedelta(days=2)), key=f"e_range_{row_id}")
                ...
                if entry_mode == "연속 기간(여러 날)":
                    if isinstance(selected_range, (tuple, list)) and len(selected_range) == 2:
                        ...
                        success, err_msg = insert_task_range(
                            ...
                            end_t.strftime("%H:%M"),
                            title, memo, 0, is_meeting, meeting_notes
                        )
                    else:
                        st.error("시작일과 종료일을 모두 선택해 주세요.")
                        success = True
